...
def speedtest():
    ...
    mydlspeed=((dlspeed / 1000 / 1000) * args.units[1], args.units[0])
    if not args.simple:
        mydlspeed
        print_()
    print_('Download: %0.2f M%s/s' % mydlspeed)
    ...
    myulspeed=((ulspeed / 1000 / 1000) * args.units[1], args.units[0])
    if not args.simple:
        print_()
    ...
    if args.share and args.mini:
        ...
        print_('Share results: %s://www.speedtest.net/result/%s.png' %
               (scheme, resultid[0]))
    return(mydlspeed,myulspeed,best['latency'])
...
def main():
    try:
        ...
        print(z)
        my_t=mytime()
        logfile.write(my_t[0]+'\t'+str(my_t[1])+'\t'+x+'\t'+y+'\t'+z+'\n')
